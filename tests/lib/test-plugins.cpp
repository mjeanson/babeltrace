...
static char *get_test_plugin_path(const char *plugin_dir, const char *plugin_name)
{
    char *ret;
    char *plugin_file_name;

    if (asprintf(&plugin_file_name, "plugin-%s." G_MODULE_SUFFIX, plugin_name) == -1) {
        abort();
    }
...
static void test_minimal(const char *plugin_dir)
{
    ...
    status = bt_plugin_find_all_from_file(minimal_path, BT_FALSE, &plugin_set);
    ok(status == BT_PLUGIN_FIND_ALL_FROM_FILE_STATUS_OK,
       "bt_plugin_find_all_from_file() succeeds with a valid file");
    ok(plugin_set, "bt_plugin_find_all_from_file() returns a plugin set");
...
static void test_sfs(const char *plugin_dir)
{
    ...
    BT_ASSERT(graph);
    graph_ret = bt_graph_add_sink_component(graph, sink_comp_class, "the-sink", NULL,
                                            BT_LOGGING_LEVEL_NONE, &sink_component);
...
static void test_create_all_from_dir(const char *plugin_dir)
{
    ...
    status = bt_plugin_find_all_from_dir(NON_EXISTING_PATH, BT_FALSE, BT_FALSE, &plugin_set);
    ok(status == BT_PLUGIN_FIND_ALL_FROM_DIR_STATUS_ERROR,
       "bt_plugin_find_all_from_dir() fails with an invalid path");
    bt_current_thread_clear_error();

    plugin_set = NULL;
    status = bt_plugin_find_all_from_dir(plugin_dir, BT_FALSE, BT_FALSE, &plugin_set);
    ok(status == BT_PLUGIN_FIND_ALL_FROM_DIR_STATUS_OK,
       "bt_plugin_find_all_from_dir() succeeds with a valid path");
    ok(plugin_set, "bt_plugin_find_all_from_dir() returns a plugin set with a valid path");
